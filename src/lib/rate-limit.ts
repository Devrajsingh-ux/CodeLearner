...
const FAIL_OPEN = process.env.RATE_LIMIT_FAIL_OPEN === 'true'; // Default: fail closed
...
function getClientId(request: NextRequest): string {
  ...
  if (accessToken) {
    // Use a hash of the token (first 16 chars) as identifier
 ...
 * SECURITY: Fails CLOSED by default when Redis is unavailable
 */
export async function rateLimit(
  ...
  try {
    ...
    // Redis unavailable - use memory fallback or fail closed
    if (FAIL_OPEN) {
      console.warn('Rate limiting: Redis unavailable, using memory fallback');
      return memoryRateLimit(clientId, config);
    }

    // FAIL CLOSED: Deny requests when Redis is unavailable in production
    ...
    // On error, use memory fallback or fail closed
    if (FAIL_OPEN) {
      return memoryRateLimit(clientId, config);
    }
