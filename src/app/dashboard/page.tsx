...
const STAT_CONFIG = [
  { icon: Flame,    label: "Day Streak",    getValue: () => `${MOCK_STREAK}`,          accent: "#f59e0b" },
  { icon: Trophy,   label: "Total XP",      getValue: () => formatNumber(MOCK_XP),     accent: "#8b5cf6" },
  { icon: Zap,      label: "Current Level", getValue: () => `${MOCK_LEVEL}`,           accent: "#06b6d4" },
  { icon: BookOpen, label: "Courses",       getValue: () => `${ENROLLED_TRACK_IDS.length}`, accent: "#10b981" },
];
...
export default function DashboardPage() {
  ...
  const recommended   = tracks.filter((t) => !ENROLLED_TRACK_IDS.includes(t.id)).slice(0, 4);
        ...
        <section className="mb-10">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-zinc-500 mb-0.5">Good to see you again</p>
              ...
              >
                ...
                <div className="relative">
                  <div
                    ...
                    style={{ background: `${accent}18`, boxShadow: `0 0 20px 0 ${accent}20` }}
                  >
                    <Icon className="h-5 w-5" style={{ color: accent }} />
                  </div>
                  <p className="text-2xl font-bold text-white tabular-nums">{getValue()}</p>
            ...
            <div>
              ...
              <div className="space-y-3">
                {enrolledTracks.map((track, idx) => {
                  ...
                  const lessonsCompleted = Math.round((pct / 100) * track.lessonsCount);
                  const isPrimary        = idx === 0;

                  return (
                    <Link key={track.id} href={`/learn/${track.slug}`} className="block group">
                      <div
                        className={[
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{ background: `linear-gradient(to bottom, ${techColor}cc, transparent)` }}
                        />
                        ...
                        <div className="flex items-center gap-4 p-5">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="mb-2 flex items-start justify-between gap-2">
                              <div>
                                <p className="font-semibold leading-tight text-white">{track.title}</p>
                                <p className="mt-0.5 flex items-center gap-1 text-xs text-zinc-500">
                                  <Clock className="h-3 w-3" />
                                  {lessonsCompleted} / {track.lessonsCount} lessons
                                </p>
                              </div>
                              <span
            ...
            <div className="rounded-2xl border border-white/6 bg-white/3 p-5">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-white">Today's Goals</span>
                </div>
              ...
              <div className="space-y-3">
                {DAILY_GOALS.map((goal) => (
                  <div key={goal.label} className="flex items-center gap-3">
                    ...
                    <span
                      className={`text-sm ${
                        goal.done ? "text-zinc-500 line-through" : "text-zinc-300"
                      }`}
                    >
                      {goal.label}
                    </span>
          ...
          <div className="space-y-5">
            ...
            <div className="relative overflow-hidden rounded-2xl border border-violet-500/20 bg-linear-to-br from-violet-900/20 to-indigo-900/10 p-5">
              ...
              <div className="relative">
                <div className="mb-1 flex items-center justify-between">
                  <span className="text-xs font-medium text-zinc-400">Level Progress</span>
                ...
                <ProgressBar value={MOCK_XP} max={XP_TO_NEXT} color="violet" size="md" />
                  ...
                  <Link key={track.id} href={`/learn/${track.slug}`} className="block group">
                    <div className="flex items-center gap-3 rounded-xl border border-white/6 bg-white/3 p-3 transition-all duration-200 hover:border-white/12 hover:bg-white/5">
                      <div
                        ...
                        style={{ boxShadow: `0 0 14px 0 ${getTechColor(track.slug)}28` }}
                      >
            ...
            <div className="rounded-2xl border border-white/6 bg-white/3 p-5">
              ...
              <p className="mt-3 text-xs text-zinc-500">
                <span className="font-medium text-white">5 days</span> active — great work!
              </p>
            </div>
