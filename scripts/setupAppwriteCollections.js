...
async function setupCollections() {
  ...
  try {
    ...
    try {
      await databases.createCollection(DATABASE_ID, 'notifications', 'notifications', [
        'read("users")',
        'create("users")',
        'update("users")',
        'delete("users")'
    ...
    console.log('📝 Adding attributes to notifications collection...');

    const notificationAttributes = [
      { key: 'recipientId', type: 'string', size: 100, required: true },
      { key: 'title', type: 'string', size: 300, required: true },
      { key: 'body', type: 'string', size: 2000, required: false },
      { key: 'link', type: 'string', size: 500, required: false },
      { key: 'unread', type: 'boolean', required: true },
      { key: 'createdAt', type: 'string', size: 50, required: false },
      { key: 'updatedAt', type: 'string', size: 50, required: false },
    ];

    for (const attr of notificationAttributes) {
      try {
        if (attr.type === 'string') {
          await databases.createStringAttribute(DATABASE_ID, 'notifications', attr.key, attr.size, attr.required);
        } else if (attr.type === 'integer') {
          await databases.createIntegerAttribute(DATABASE_ID, 'notifications', attr.key, attr.required);
        } else if (attr.type === 'float') {
          await databases.createFloatAttribute(DATABASE_ID, 'notifications', attr.key, attr.required);
        } else if (attr.type === 'boolean') {
          await databases.createBooleanAttribute(DATABASE_ID, 'notifications', attr.key, attr.required);
        }
        console.log(`   ✅ ${attr.key} (${attr.type})`);
    ...
    try {
      await databases.createIndex(DATABASE_ID, 'notifications', 'recipient_index', 'key', ['recipientId']);
      console.log('   ✅ notifications.recipientId index');
    } catch (e) {
      if (e.code === 409) {
        console.log('   ⏭️  notifications.recipientId index already exists');
      }
    }
